    ...
    match es!({
        ...
        struct Arguments {
            ...
            #[arg(
                short,
                long,
                help = "If specified, only generate code for the listed resources. This does not include the provider prefix (stripe_). Prefix with data_ for data sources."
            )]
            include: Vec<String>,
        }
        let args = Arguments::parse();

        let (vendor, shortname) = args
            .provider
            .split_once("/")
            .unwrap_or_else(|| ("hashicorp".into(), &args.provider));
        ...
        let provider_name: Ident;
        {
            ...
            out.push(quote! {
                ...
                impl #provider_builder_name {
                    pub fn build(self, _provider_type: &#provider_type_name, stack: &mut Stack) -> Rc<#provider_name> {

...
use super::{
    generateshared::{
        add_path, generate_field, generate_simple_type, sanitize, to_camel, TopLevelFields,
    },
    sourceschema::{AggCollType, AggCollTypeKey, AggObjType, Value, ValueSchema},
};
...
pub fn generate_fields(
    ...
    for (k, v) in fields {
        ...
        // Only generate readers for primitive fields -- I'm not sure where
        // collection fields would be useful (without for-each) and references
        // only work with Primitive atm (which doesn't support collections -- not
        // sure how hard that would be to add, although it could be simple).
        // - Note that this may cause some unused types to be generated here (computed
        // collections, not used above and not used below)
        let generate_ref = self_has_identity
            && match &v.r#type {

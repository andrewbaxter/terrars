...
impl Stack {
    ...
    pub fn serialize(&self) -> Result<Vec<u8>, StackError> {
        ...
        for p in &self.provider_types {
            if required_providers
                .insert(p.extract_tf_id(), p.extract_required_provider())
                .is_some()
            {
                Err(StackError::Duplicate(
                    ComponentType::ProviderType,
                    p.extract_tf_id(),
                ))?;
            }
        }
        ...
        for v in &self.variables {
            if variables
                .insert(v.extract_tf_id(), v.extract_value())
                .is_some()
            {
                Err(StackError::Duplicate(
        ...
        for o in &self.outputs {
            if outputs
                .insert(o.extract_tf_id(), o.extract_value())
                .is_some()
            {
                Err(StackError::Duplicate(
    ...
    /// are provided, they must be a single-level struct where all values are primitives
    /// (i64, f64, String, bool).
    pub fn run<V: Serialize>(
        &self,
        path: &Path,
        variables: Option<&V>,
        mode: &str,
    ...
    /// Gets the current outputs from an applied stack. `path` is the directory in which
    /// the .tf.json file was written. The output struct must be a single level and only
    /// have primitive values (i64, f64, String, bool).
